...
int main(void)
{
  ...
  // main
	long count = 0;
  // [A,B]までの総和を考えて、(A+B)(B-A+1)=2N (A<=B)
  // A+b=x, B-A+A=yとすると、xとyはともに2Nの約数
  // xmod2==ymod2だと解なしのため、xとyのどちらかに素因数2が集まる
  while(N%2==0)		// 素因数2を片方に集める
		N /= 2;
	// あとは、2以外の素因数の分配の仕方がxとyの組み合わせの個数/2となる
	long n = sqrt(N);
