...
int main(void)
{
  ...
  long N,M;
  cin >> N >> M;
  auto A = vector<long>(M);
  auto B = vector<long>(M);
  for(int ix=0; ix!=M; ++ix){
	  cin >> A[ix] >> B[ix];
  }
  long K;
  cin >> K;
  auto C = vector<long>(K);
  auto D = vector<long>(K);
  ...
  for(int now=0; now<limit; ++now){
	  ...
	  auto sara = vector<long>(N, 0);
		...
		for(int jx=0; jx!=M; ++jx){
			if(sara[A[jx]-1]!=0 && sara[B[jx]-1]!=0)
				tmp += 1;
		}

		countmax = max(countmax, tmp);
  }
