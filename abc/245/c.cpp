...
int main(void)
{
  ...
  // main
  // dp[i][j]: i番目までを、(false: A終わり | true: B終わり)でいけるか
  vector<vector<bool>> dp(N, vector(2, false));
  dp[0][0] = true;
  dp[0][1] = true;
  ...
  rep(ix, N) {
    if (ix == 0) continue;
    if (dp[ix-1][0]) {
      long diffA = A[ix-1] - A[ix];
      long diffB = A[ix-1] - B[ix];
      if (abs(diffA) <= K) dp[ix][0] = true;
      if (abs(diffB) <= K)  dp[ix][1] = true;
    }
    if (dp[ix-1][1]) {
      long diffA = B[ix-1] - A[ix];
      long diffB = B[ix-1] - B[ix];
      if (abs(diffA) <= K) dp[ix][0] = true;
      if (abs(diffB) <= K)  dp[ix][1] = true;
    }
  }
