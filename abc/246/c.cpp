...
int main(void)
{
  ...
  // main
  for (int ix = 0; ix != N && K >= 0; ++ix) {
    const ulong consume = min(A[ix] / X, (ulong)K);
    A[ix] -= consume * X;
    K -= consume;
  }
  assert(K >= 0);
  ...
  //show(A);
  sort(A.begin(), A.end(), [](ulong a, ulong b) {
    return a > b;
  });
  //show(A);

  //cout << K << endl;
  const ulong sum = accumulate(A.begin() + min(A.size(), (ulong)K), A.end(), 0UL);
