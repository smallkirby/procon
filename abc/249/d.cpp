...
int main(void)
{
  ...
  ulong N; cin >> N;
  vector<ulong> A;
  vector<unsigned> mapping(upper+1, 0);
  rep(ix, N) {
    ulong tmp;
    cin >> tmp;
    if (mapping[tmp] == 0) A.push_back(tmp);
    ++mapping[tmp];
  }
  sort(A.begin(), A.end());
  ulong _max = A[A.size() - 1];

  // main
  ulong result = 0;
  if (A.size() >= 2) {
    for (int ix = 0; ix != A.size(); ++ix) {
      for (int jx = ix + 1; jx <= A.size() - 1; ++jx) {
        ulong tmp = A[ix] * A[jx];
        if (tmp <= upper && mapping[tmp] != 0) {
          result += mapping[tmp] * 2 * ncr(mapping[A[ix]], 2);
        }
        if (tmp > _max) {
          break;
        }
  ...
  rep(ix, A.size()) {
    ulong tmp = A[ix] * A[ix];
    if (tmp <= upper && mapping[tmp] != 0) {
      result += mapping[tmp] * mapping[tmp];
    }
  }
