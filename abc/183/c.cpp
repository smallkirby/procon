...
auto remain = vector<int>();
long nowtime = 0;
void func(int now, vector<vector<long>> &V){
  int els = remain.size();
  if(els == 1){
    if(nowtime + V[now][0] == K)
      ++counter;
    //cout << "now: "<<nowtime + V[now][0]<<endl;
    return;
  }

  for(int ix=1; ix!=els; ++ix){
    auto poped = remain[ix];
    nowtime += V[now][poped];
    remain.erase(remain.begin() + ix);
    func(poped, V);
    remain.insert(remain.begin() + ix, poped);
    nowtime -= V[now][poped];
  }
}
...
int main(void)
{
  // input
  cin >> N >> K;
  auto V = vector<vector<long>>(N, vector<long>(N, 0));
  for(auto&& vrow: V){
    for(auto&& v: vrow){
      cin >> v;
    }
