...
bool f(pair<long,long> l, pair<long,long> r){
...
int main(void)
{
  ...
  long N;
  cin >> N;
  auto x = vector<pair<long,long>>(N);
  long aoki = 0;
  long takahashi = 0;
  for(int ix=0; ix!=N; ++ix){
    cin >> x[ix].first >> x[ix].second;
    aoki += x[ix].first;
  }

  sort(x.begin(), x.end(), f);
  //for(int ix=0;ix!=N;++ix){
  //  cout << "("<<x[ix].first<<","<<x[ix].second<<")"<<endl;
  //}

  // main
  int ix;
  for(ix=0; ix!=N; ++ix){
    takahashi += x[ix].first + x[ix].second;
    aoki -= x[ix].first;
    if(takahashi > aoki){
      break;
    }
