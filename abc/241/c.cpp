...
int main(void)
{
  ...
  rep(n, N) {
    rep(m, N) {
      ...
      if (m + 6 <= N) {
        auto c = 0;
        rep(ix, 6) if(board[n][m+ix] == '#') ++c;
        if (c>=4) print_exit(true);
      }
      // col
      if (n + 6 <= N) {
        auto c = 0;
        rep(ix, 6) if(board[n+ix][m] == '#') ++c;
        if (c>=4) print_exit(true);
      }
      // slash right up
      if ((m + 6 <= N) && (5 <= n)) {
        auto c = 0;
        rep(ix, 6) if(board[n-ix][m+ix] == '#') ++c;
        if (c>=4) print_exit(true);
      }
      // slash right down
      if ((n + 6 <= N) && (m + 6 <= N)) {
