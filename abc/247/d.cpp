...
int main(void)
{
  ...
  getline(cin, query[0]);
  rep(ix, Q) {
    getline(cin, query[ix]);
  }

  // main
  queue<pair<ulong, ulong>> que; // num,val
  pair<ulong, ulong> tmp = make_pair(0,0);
  rep(ix, Q) {
    if (query[ix][0] == '1') {
      ulong x,c;
      sscanf(query[ix].c_str(), "1 %lu %lu", &x, &c);
      que.push(make_pair(c, x));
    } else {
      ulong l = stoul(query[ix].substr(2));
      ulong sum = 0;

      if (tmp.first != 0) {
        ulong use_num = min(l, tmp.first);
        l -= use_num;
        tmp.first -= use_num;
        sum += use_num * tmp.second;
      }
      while (l > 0) {
