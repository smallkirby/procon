...
int main(void)
{
  ...
  */
  ...
  int N, W;       // 荷物の個数, 重さの限界
  cin >> N >> W;
  vector<vector<int>> nup(N+1, vector<int>(W+1, -1));     // 0~限界まで. 荷物iまでを見た時にwを超えない最大の価値がnup[i][w]
  vector<struct bag> B(N);     // 価値、重さ
  for(int ix=0; ix!=N; ++ix){
    cin >> B[ix].val >> B[ix].weight;
  }
  for(int w=0; w!=W+1; ++w){
    nup[0][w] = 0;
  }
  ...
  for(int n=0; n!=N; ++n){
    for(int w=0; w!=W+1; ++w){
      if(w >= B[n].weight)        // まだカバンに入れられる
        nup[n+1][w] = max(max(nup[n][w - B[n].weight] + B[n].val, nup[n][w]), nup[n+1][w - B[n].weight] + B[n].val);  // 3項目が同じものを重複して取ることに相当。なんか、分かるようで分からんような気もするねんな。
      else{
        nup[n+1][w] = nup[n][w];
      }
    }
