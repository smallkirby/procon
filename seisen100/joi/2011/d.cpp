...
int main(void)
{
  ...
  vector<vector<long>> dp(nums.size()-1+1, vector<long>(20+1, 0));
  dp[1][nums[0]] = 1;

  // main: なんかオリジンわかりにくくなっちゃったな
  for(int ix=1; ix<nums.size(); ++ix){      // 何文字目まで見るか
    for(int jx=0; jx<=20; ++jx){           // 合計jxにする
      //if(jx=11 && ix==2)
      //  cout << "num(2)=" << nums[ix] << " dp[ix-1][jx-nums[ix-1]]:"<<dp[ix-1][jx-nums[ix-1]]<<endl;
      if(jx - nums[ix-1] >= 0)
        dp[ix][jx] += dp[ix-1][jx-nums[ix-1]];    // 足し算パターン
      if(jx + nums[ix-1] <= 20)
        dp[ix][jx] += dp[ix-1][jx+nums[ix-1]];    // 引き算パターン
    }
  }

  // print
  //show2(dp);
  cout << dp[nums.size()-1][right] << endl;
}